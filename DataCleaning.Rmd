...
df_unite %>% dplyr::group_by(Backchannel, Language) %>% dplyr::summarise(d=n())
df_unite %>% dplyr::group_by(Repair, Language) %>% dplyr::summarise(d=n())
df_unite <- df_unite %>% 
  dplyr::group_by(ID) %>% 
...
#Adding Lexical alignment and structural alignment to danish sentences
```{r}
...
#Adding Lexical alignment and structural alignment to Norwegian sentences
```{r}
#Subsetting Danish data
df_NO <- df_unite %>% 
...
#Making specific tokens for different versions of 'nå'
```{r}
...
```{r}
df_DK$Backchannel <- as.numeric(df_DK$Backchannel)
df_NO %>% dplyr::group_by(Pair) %>% dplyr::summarise(rep=sum(Backchannel), n=n()) %>% mutate(percent = rep/n*100)
df_DK %>% dplyr::group_by(Pair) %>% dplyr::summarise(bc=sum(Backchannel, na.rm=T), n=n()) %>% mutate(percent = rep/n*100)

df_DK$index <- 1:nrow(df_DK)

df_DK <- read_csv("df_DK.csv")
...
sum(Balance_df$n_pair)
mean(Balance_df$group_freq)
sd(Balance_df$group_freq)
