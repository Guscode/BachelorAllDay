...
for(i in 1:length(folds)){
  ...
  freq99_t <- round(0.001*nrow(test.data)) #getting numeric value for using 99.9% most frequent words
  ...
  freq99 <- round(0.001*nrow(train.data))
...
for(i in 1:length(folds)){
  ...
  #Creating DTM from test data
  freq99_t <- round(0.001*nrow(test.data)) #getting numeric value for using 99% most frequent words
  corp_t <- corpus(as.character(test.data$Transcription))
  dtm_t <- dfm(corp_t)
  dtm_t <- dfm_trim(dtm_t, min_docfreq = freq99_t, verbose = TRUE)
  dtm_t <- convert(dtm_t, to="tm")
