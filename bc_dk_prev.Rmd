...
#loading data
dk_bc_url <- getURL("https://raw.githubusercontent.com/Guscode/BachelorAllDay/master/df_DK.csv")
dk_bc <- read.csv(text = dk_bc_url)
...
pairs <- levels(as.factor(dk_bc$Pair))[1:5]
dk_bc <- dk_bc[dk_bc$Pair %in% pairs,]

#Looping through all pairs
for(i in 1:length(pairs)){
  
  #Splitting train and test data
  train.data <- dk_bc[!dk_bc$Pair==pairs[i],]
  test.data <- dk_bc[dk_bc$Pair==pairs[i],]
  ...
  corp_prev <- corpus(as.character(train.data$preceeding), text_field = "Reviews")
...
```{r}
dk_bc$resis <- paste(dk_bc$binary.x, dk_bc$rf_pred, sep="")
dk_bc_prev_res <- dk_bc
dk_bc %>% dplyr::group_by(resis) %>% dplyr::summarise(n =n(), n_align = sum(Backchannel_alignment))
